...
/* Here the doStep function comes*/
	...
	M_DiscreteGrippingArm_input_Enum_Channel* start_DiscreteGrippingArm = channels->start_DiscreteGrippingArm;
{
	...
	//while (!terminate__) { // Terminate of the FMU instead
		{
			bool inputdone = false;
			//while (!inputdone) {
				{
					char _s0[256];
					////sprintf(_s0, "%s", "Enter an event - Control loop:");
					// Attribute update
					bool gripperOpened = sm_inputstate->gripperOpened;
					bool robotStopped = sm_inputstate->robotStopped;
					bool closeGripperCommand = sm_inputstate->closeGripperCommand;
					bool collision = sm_inputstate->collision;
					bool feasibleMoveDiscreteCommand = sm_inputstate->feasibleMoveDiscreteCommand;
					bool gripperClosed = sm_inputstate->gripperClosed;
					bool nonfeasibleMoveDiscreteCommand = sm_inputstate->nonfeasibleMoveDiscreteCommand;
					bool moveCompleted = sm_inputstate->moveCompleted;
					bool openGripperCommand = sm_inputstate->openGripperCommand;
					bool moveDiscreteCommand = sm_inputstate->moveDiscreteCommand;
					//_s0
					//strncpy(_s0, "moveDiscreteCommand", 256);
					printf("%s", _s0);
				}
				char* _event_;
				char _temp_0[256];
	...
	//while (!terminate__) {
		{
			...
			//} // Update Santiago - Control thread
...
void init(sm_memory* sm_memory, sm_inputstate* sm_inputstate, sm_state* sm_state ) {
	log_file = fopen("test.log", "w");
	sm_memory->MOTION_VEL= 0.0;
	sm_memory->opening_diameter=0.0;
	sm_memory->PI=3.1416;
	sm_memory->av=0.0;
	sm_memory->TARGET_X=0;
	sm_memory->TARGET_Y=0;
	sm_memory->TARGET_Z=0;
	sm_memory->closing_speed=0.0;
	sm_memory->closing_force=0.0;
	sm_memory->closing_diameter=0.0;
	sm_memory->lv=0.0;
	sm_memory->opening_speed=0.0;
	sm_memory->opening_force=0.0;
	//comp->fmiBuffer.realBuffer[43] = comp->sm_memory.margs;
	sm_memory->margs.target_X=0;
	sm_memory->margs.target_Y=0;
	sm_memory->margs.target_Z=0;

	sm_inputstate->collision=false;
	sm_inputstate->gripperOpened=false;
	sm_inputstate->closeGripperCommand=false;
	sm_inputstate->robotStopped=false;
	sm_inputstate->moveCompleted=false;
	sm_inputstate->gripperClosed=false;
	sm_inputstate->feasibleMoveDiscreteCommand=false;
	sm_inputstate->openGripperCommand=false;
	sm_inputstate->moveDiscreteCommand=false;
	sm_inputstate->nonfeasibleMoveDiscreteCommand=false;
	//comp->fmiBuffer->realBuffer[11] = comp->sm_inputstate->moveDiscreteCommand_value;
	sm_inputstate->moveDiscreteCommand_value.target_X=0;
	sm_inputstate->moveDiscreteCommand_value.target_Y=0;
	sm_inputstate->moveDiscreteCommand_value.target_Z=0;
