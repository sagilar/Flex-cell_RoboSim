...
/***** Specific to FMI *****/
ModelData* fmiModel; //not needed for now
...
bool step(Infrastructure* state) {
	printf("Running step from the DiscreteGrippingArm module\n");
...
int main(int argc, char* argv[]) {
...

	{
		...
		while (!terminate__) {
			...
			//terminate__ = step(state,fmiModel);
		}
	}
...
void init(ModelData* comp) { //Replace for the main function
		fmiModel = comp;
		setStartValues(comp);
		int tmp_x = comp->TARGET_X;
		int tmp_y = comp->TARGET_Y;
		int tmp_z = comp->TARGET_Z;
		printf("TargetX: %d\n", tmp_x);
		printf("TargetY: %d\n", tmp_y);
		printf("TargetZ: %d\n", tmp_z);
		...
		{
			bool terminate__ = false;
			printf("Before initializing the step function\n");
			while ((!comp->terminateSimulation) || (!terminate__)) {
			...
			printf("After calling the step function\n");
		}
		clean(state);
...
ModelData* tick(ModelData* comp){
	...
	fmiModel = comp;
	printf("target_Y_tick: %d\n",fmiModel->target_Y);
	printf("target_Z_tick: %d\n",comp->target_Z);
	//update comp to the main function
	//send back the updated comp


	/*Infrastructure* state2 = (Infrastructure*)malloc(sizeof(Infrastructure));

	initialiseProgrammingEnvironment();
	initialiseConcurrencyInfrastructure(state2);
	{
		bool terminate__ = false;
		printf("Before initializing the step function\n");
		if ((!comp->terminateSimulation) || (!terminate__)) {
			printf("In if loop\n");
			//terminate__ = step(state,comp);
			terminate__ = step(state);
		}
		printf("After calling the step function\n");
	}*/
